...
int create_znode(char *server_addr_port){
    ...
    path = malloc(node_buffer_len - 7); // -7 because it doesnt have "/chain/"
    if (path == NULL){
        free(node_buffer);
        return -1;
    }
    strcpy(path, node_buffer + 7);
    free(node_buffer);
    printf("\n");
    printf("%s\n", path);
    printf("\n");
...
int invoke(MessageT *msg){
    if (msg == NULL || msg->opcode == MESSAGE_T__OPCODE__OP_ERROR || tree == NULL)
        return -1;
